...
//				EditorGUILayout.HelpBox("Use (Tools\\Parse Terms) to find how many times each of the Terms are used", UnityEditor.MessageType.Info);
		...
		void OnGUI_Keys_ListSelection ( int KeyListFilterID )
		{
			...
			// Terms Filter
			{
                ...
                KeyList_Filter = GUITools.TextField( mKeyListFilterRect, KeyList_Filter, 255, GUI.skin.GetStyle( "ToolbarSearchTextField" ), KeyListFilterID );
			}



			if (GUILayout.Button( string.Empty, string.IsNullOrEmpty( KeyList_Filter ) ? "ToolbarSearchCancelButtonEmpty" : "ToolbarSearchCancelButton", GUILayout.ExpandWidth( false ) ))
			{
